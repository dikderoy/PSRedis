 ...
 *
 * Adapts the RedisGuard\Client objects to Predis
 * @link
 *
 * @package RedisGuard\Client\Adapter
 */
class PredisClientAdapter extends AbstractClientAdapter implements IClientAdapter
{
	/**
	...
	{
		list($masterIpAddress, $masterPort) = $this->getPredisClient()
												   ->sentinel(SentinelCommand::GETMASTER, $nameOfNodeSet);

		if (!empty($masterIpAddress) AND !empty($masterPort)) {
			return new \RedisGuard\Client($masterIpAddress, $masterPort, new PredisClientAdapter($this->predisClientFactory, Client::TYPE_REDIS));
		}
