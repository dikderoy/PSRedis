...
class IncrementalTest extends \PHPUnit_Framework_TestCase
{
    public function testThatWeCanChooseTheInitialBackoffValue()
    {
        ...
        $backoff = new IncrementalBackOff(500000, 1);
        $this->assertEquals(500000, $backoff->getBackOffInMicroSeconds(), 'We choose the initial backoff to be 500000');

        $this->setExpectedException('\\RedisGuard\\Exception\\InvalidProperty', 'The initial backoff cannot be smaller than zero');
        new IncrementalBackOff(-500000, 1);
    }

    public function testBackoffWithoutMultiplying()
    {
        $backoff = new IncrementalBackOff(1, 1);
    ...
    {
        $backoff = new IncrementalBackOff(1, 2);
    ...
    {
        $backoff = new IncrementalBackOff(2, 1.5);
    ...
    {
        $backoff = new IncrementalBackOff(0, 0);
